...
32x-maptest: $(BUILD_DIR)/aerobiz-ultimate-maptest.32x

$(BUILD_DIR)/aerobiz-ultimate-maptest.32x: $(BUILD_DIR)/32x_boot_maptest.bin $(GAME_HALF)
	@echo "==> Assembling world map test cartridge..."
...
$(BUILD_DIR)/32x_boot_maptest.bin: $(BOOT_SRC) $(BOOT_INC) $(MARS_INIT_BIN) $(MARS_INIT_INC) $(SH2_IMAGE_BIN) $(SH2_IMAGE_INC) $(BUILD_DIR)/map_asset.bin | $(BUILD_DIR)
	...
	$(ASM) $(ASMFLAGS) -DMAPTEST=1 -DMAPASSET=1 -o $@ $<

32x-zoomtest: $(BUILD_DIR)/aerobiz-ultimate-zoomtest.32x

$(BUILD_DIR)/aerobiz-ultimate-zoomtest.32x: $(BUILD_DIR)/32x_boot_zoomtest.bin $(GAME_HALF)
	@echo "==> Assembling map zoom cartridge..."
	@cat $(BUILD_DIR)/32x_boot_zoomtest.bin $(GAME_HALF) > $@
	@echo "==> Build complete: $@"

$(BUILD_DIR)/32x_boot_zoomtest.bin: $(BOOT_SRC) $(BOOT_INC) $(MARS_INIT_BIN) $(MARS_INIT_INC) $(SH2_IMAGE_BIN) $(SH2_IMAGE_INC) $(BUILD_DIR)/map_asset.bin | $(BUILD_DIR)
	@echo "==> Assembling 32X boot half, map zoom (\$$880000)..."
	$(ASM) $(ASMFLAGS) -DZOOMTEST=1 -DMAPASSET=1 -o $@ $<
...
32x-nolz: $(BUILD_DIR)/aerobiz-ultimate-nolz.32x

$(BUILD_DIR)/aerobiz-ultimate-nolz.32x: $(BOOT_HALF) $(BUILD_DIR)/32x_game_nolz.bin
	@echo "==> Assembling 32X cartridge, LZ on the 68000..."
	@cat $(BOOT_HALF) $(BUILD_DIR)/32x_game_nolz.bin > $@
	@echo "==> Build complete: $@"

$(BUILD_DIR)/32x_game_nolz.bin: $(GAME_SRC) $(SHARED_SRCS) | $(BUILD_DIR)
	@echo "==> Assembling 32X game half, stock 68000 decompressor (\$$900000)..."
	$(ASM) $(ASMFLAGS) -DNOSH2LZ=1 -o $@ $<
